...
const CustomChart = ({
  ...
  if (variant === "line") {
    ...
    const bar = useStackedBarController(
      gridElement,
      values,
      maxValue,
      fixedBarWidth
  ...
  return (
    ...
    >
      <div className="flex">
        ...
        >
          <div ref={canvasElement} className="absolute"></div>
          <div
            ref={gridElement}
          ...
          >
            ...
            <div
              className="custom-chart_visual absolute w-full"
                  ...
                  {itemValues.map((data, index) => (
                    <>
                      <div
                        ...
                        style={{ bottom: "0%", left: data.left + "px" }}
                      >
              ...
              {variant === "stacked-bar" && (
                <>
                  {stackedValues.map((data, index) => (
                    <div key={index}>
                      <div
                        className="absolute flex flex-col-reverse gap-[6px]"
                        style={{ bottom: "0%", left: data.left + "px" }}
                      >
                        {data.items.map((item, index2) => (
                          <BarChartItem
      ...
      <div className="x-container min-h-[1.751rem] max-h-[1.751rem] border-t border-b border-[var(--base-bg3)] flex items-center">
        ...
        <div
          className={`custom-chart_x flex grow justify-between py-1.5 ${xClassname}`}
        >
          {labels.map((item) => (
            <div key={item} className="text-xs text-gray6 font-extralight">
              {item}
            </div>
          ))}
        </div>
